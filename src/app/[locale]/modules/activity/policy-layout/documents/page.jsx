...
export default function Dividend() {
    ...
    const t = useTranslations("Activity-Policy-Documents")

    const [data, setData] = useState(null)
    useEffect(() => {
           axios.get(`https://tashbus.uz/${document.cookie.slice(12)}/api/document/yoshlar_siyosati`).then(response => setData(response.data))
    }, [])
...
//  }
    ...
    return <div className={cls.facts}>
        ...
        <div className="hero-dinamic-router">
            ...
            <Link className='hero-dinamic-route-link' href={'/modules/activity/policy-layout/documents'}>{t('route2')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={'/modules/activity/policy-layout/documents'}>{t('route3')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={'/modules/activity/policy-layout/documents'}>{t('route4')}</Link>
        </div>
