...
export default function Affiliates() {
    ...
    const t = useTranslations("Corparate-shareholders-Affili")

    const [data, setData] = useState(null)
    useEffect(() => {
           axios.get(`https://tashbus.uz/${document.cookie.slice(12)}/api/document/affillangan_shaxslar`).then(response => setData(response.data))
    }, [])
...
//  }
    ...
    return <div className={cls.facts}>
        ...
        <Link className='hero-dinamic-route-link' href={'/'}>{t('route1')}</Link>
            ...
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/shareholders/affiliates'}>{t('route2')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/shareholders/affiliates'}>{t('route3')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/shareholders/affiliates'}>{t('route4')}</Link>
        </div>
