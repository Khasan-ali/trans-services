...
export default function Facts() {
    ...
    const t = useTranslations("Corparate-Disclosure-Facts")

    const [data, setData] = useState(null)
    useEffect(() => {
           axios.get(`https://tashbus.uz/${document.cookie.slice(12)}/api/document/muhim_faktlar?p=2&page_size=6`).then(response => setData(response.data))
    }, [])
    ...
    return <div className={cls.facts}>
        ...
        <div className="hero-dinamic-router">
            ...
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/disclosure/facts'}>{t('route2')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/disclosure/facts'}>{t('route3')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/disclosure/facts'}>{t('route4')}</Link>
        </div>
