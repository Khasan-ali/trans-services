...
export default function IssuerReports() {

    const t = useTranslations("Corparate-Disclosure-Audit-Summary")

    const [data, setData] = useState(null)
    useEffect(() => {
           axios.get(`https://tashbus.uz/${document.cookie.slice(12)}/api/document/audit_xulosasi`).then(response => setData(response.data))
    }, [])
...
//  }
    ...
    return <div className={cls.facts}>
        ...
        <div className="hero-dinamic-router">
            ...
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/disclosure/audit-summary'}>{t('route2')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/disclosure/audit-summary'}>{t('route3')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/disclosure/audit-summary'}>{t('route4')}</Link>
        </div>
