...
export default function InternalDocuments() {
    ...
    const t = useTranslations("Corparate-Internal-Documents")

    const [data, setData] = useState(null)
    useEffect(() => {
           axios.get(`https://tashbus.uz/${document.cookie.slice(12)}/api/document/jamiyat_ichki_hujjatlari`).then(response => setData(response.data))
    }, [])
...
//  }
    ...
    return <div className={cls.facts}>
        ...
        <div className="hero-dinamic-router">
            ...
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/corparate-layouts/internal-documents'}>{t('route2')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/corparate-layouts/internal-documents'}>{t('route3')}</Link>
        </div>
