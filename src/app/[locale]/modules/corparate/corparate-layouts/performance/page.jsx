...
export default function Performance() {
    ...
    const t = useTranslations("Corparate-Performance")

    const [data, setData] = useState(null)
    useEffect(() => {
           axios.get(`https://tashbus.uz/${document.cookie.slice(12)}/api/document/samaradorlik`).then(response => setData(response.data))
    }, [])
...
//  }
    ...
    return <div className={cls.facts}>
        ...
        <div className="hero-dinamic-router">
            ...
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/corparate-layouts/performance'}>{t('route2')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={'/modules/corparate/corparate-layouts/performance'}>{t('route3')}</Link>
        </div>
