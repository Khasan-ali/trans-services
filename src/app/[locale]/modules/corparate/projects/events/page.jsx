...
export default function DetailInvestment({ params }) {
    ...
    const t = useTranslations("Corparate-Projects-Purchaches")

    const paramData = params.id ? params.id : ''

    const [data, setData] = useState(null)
    useEffect(() => {
           axios.get(`https://tashbus.uz/${document.cookie.slice(12)}/api/post/detail/ozbekiston-respublikasi-mustaqilligining-31-yillik`).then(response => setData(response.data))
    }, [])
    ...
    return <div className={cls.detail}>
        ...
        <div className="hero-dinamic-router">
            ...
            <Link className='hero-dinamic-route-link' href={`/modules/corparate/projects/events`}>{t('route2')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={`/modules/corparate/projects/events`}>{t('route3')}</Link>
            <Image className='hero-dinamic-route-images' src={NextImg} alt='next images' />
            <Link className='hero-dinamic-route-link' href={`/modules/corparate/projects/events`}>{t('route4')}</Link>
        </div>
        ...
        <img className={cls.detail_image} src={data?.images[0]?.image?.src.split('http').join('https')} alt="detail images" />
        <a className={cls.detail_link} href={data?.file} download={data?.file} target='_blanc'>
            <Image src={DocumentImg} alt='icon'/>   
            <p className={cls.detail_link_text}>{t('link-text')}</p>
        </a>

        <p className={cls.detail_body} dangerouslySetInnerHTML={{__html: `<p>${data?.body}</h1>` }} />
    </div>
}