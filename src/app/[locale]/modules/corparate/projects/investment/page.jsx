...
export default function Investment() {
    ...
    useEffect(() => {
           axios.get(`https://tashbus.uz/${document.cookie.slice(12)}/api/post/investitsion?p=1&page_size=12`).then(response => setData(response.data))
    }, [])
        ...
        <ul className={cls.investment_list}>
            {data?.results?.map((element) => (
                <li className={cls.investment_item} key={element?.slug}>
                    ...
                    href={`/modules/corparate/projects/investment/${element?.slug}`}>
